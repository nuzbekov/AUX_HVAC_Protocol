...
>
> Что именно бит включает, определить не удалось: при `EN` = `0x80` ни вентилятор, ни байты статуса внешнего блока не отреагировали (`MODE` остался `0x24`, `FSPD` `0x00`). Но на стенде не было ни воды, ни двигателя, а комнатная температура была 25 °C, так что функция вроде защиты от замерзания или продувки просто не имела повода сработать.
