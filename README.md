...
Спасибо участникам чата [AUX-based air conditioner (ESPHome)](https://t.me/aux_ac) в Telegram за идеи, логи, обратную связь.  
Отдельная безмерная благодарность [Brokly](https://github.com/Brokly). Без него значительная часть протокола долго бы оставалась загадкой.

# Реализация на Python

В папке [`python/`](python/) лежит кодер/декодер протокола по этому описанию,
скрипт опроса COM-порта и офлайн-инструмент для проверки протокола и разбора
записанных дампов линии. Подробности — в [python/README.md](python/README.md).

Быстрая проверка того, что кодек соответствует этому описанию (железо не нужно,
зависимостей нет):

```
python python/aux_tool.py selftest
```

Там же разобран **второй интерфейс этих систем — шина RS485** центрального
управления. Это отдельный протокол, с UART-протоколом wifi-модуля у него нет
ничего общего: 9600/8-N-1, кадр со стартовым байтом `0x7E`, адресами и
CRC16/MODBUS. Кадровый уровень расшифрован и проверен на дампе шины
контроллера фанкойла AUX. Из данных расшифрованы состояние блока
(питание, режим, скорость вентилятора, дисплей, уставка) и датчики `ROOM` и
`PIPE`; коды режимов и скоростей на шине **те же, что в UART-протоколе**.
Подробности и порядок дальнейшего реверса — в
[python/README.md](python/README.md#rs485).

> **Измерено на контроллере фанкойла AUX:** по шине RS485 температура
> теплообменника отдаётся **с разрешением 0,1 градуса** — в отличие от
> байта 17 UART-протокола, где тот же датчик лежит с разрешением 1 градус.
> Проверено прогревом датчика `PIPE` феном: в регистре 13 кадра `CMD=0x02`
> значение прошло `24.7` → `74.0`, и никакой другой регистр при этом не
> изменился.
...
| `0x07` | [<=] | 4, 15 или 24 байта; подробнее [в описании пакета TYPE = `0x07`](#packet_type_info) |
| `0x09` | [=>] | 0 байт |
| `0x09` | [<=] | 1 байт |
| `0x0B` | [=>] | 2 байта |
...
Расчет CRC производится по следующему алгоритму:
- Если длина данных в байтах нечетная, то данные дополняются в конце нулевым байтом. Каждая пара байт данных берется для расчета как 16-битное число.
- Последовательно суммируем все 16-битные числа. Важно помнить, что сумма вполне может быть 32-разрядной.
- К младшим 16 битам получившейся суммы прибавляются старшие 16 бит (сумма возвращается к исходному 16-битному формату, но данные в старших разрядах не теряются).
- Дальше полученное значение побитово инвертируется. В итоге получаются 2 байта чексуммы для пакета.
...
- `0x01` (**направление:** входящий, исходящий); [подробнее](#packet_type_ping).  
  ...
  > **Измерено на контроллере фанкойла AUX:** период ping здесь **5 секунд**, а не 2,963. Замерено по 18 пакетам подряд. То есть период зависит от типа устройства и на него завязываться не стоит.
...
##### Байт 9 (?X)
непонятен. Ранее предполагалось, что этот же байт возвращается во входящем пакете TYPE = `0x07` в байте 8. **Проверено — не возвращается.**

> **Измерено на контроллере фанкойла AUX:** в байте 9 исходящего пакета перебраны значения `0x00`, `0x01`, `0x02`, `0x55`, `0xAA`, `0xFF` — по 6 вариантов для CMD = `0x11` и для CMD = `0x21`. Кондиционер ответил на все 12 запросов, и во всех ответах **байт 8 был равен `0x01`** независимо от переданного значения.
>
> Значит, байт 8 входящего пакета с байтом 9 исходящего никак не связан, и идентификатором запроса этот байт не является. Заодно видно, что произвольное значение в байте 9 обработку запроса не ломает: ответ приходит нормальный.
...
Ранее предполагалось, что это тот же байт, который передает wifi-модуль в пакете TYPE = `0x06` в байте 9 (?x). **Эксперимент это опроверг:** что бы ни передавалось в байте 9 запроса, здесь остаётся `0x01`. Подробности [в описании байта 9 пакета TYPE = `0x06`](#packet_type_cmd_b09).
...
**Бит 7:** возможно, класс устройства: для стационарных устройств тут была `1`, для мобильного кондиционера Ballu в этом бите `0`.

**Бит 6:** не известно, для всех моделей был `1`.

**Бит 5 (INV):** тип кондиционера - `1` для инверторных сплитов, `0` для сплитов on-off (для мобильного кондиционера Ballu тут тоже был `0`).
...
> **Измерено на контроллере фанкойла AUX:** здесь бит **VL тоже инвертирован**, как у Rovex. При остановленных шторках байт 11 равен `0x25`, после команды на качание (биты UD байта 10 сброшены в `000`) становится `0x21` — то есть бит VL гаснет именно при включении качания. Инверсия этого бита встречается не только у Rovex.
>
> Бит **LON** при этом остаётся `0` в обоих положениях и на состояние шторок не реагирует.
...
> **Измерено на контроллере фанкойла AUX:** значения `0x02` (LOW) и `0x06` (HIGH) подтверждены — они появляются здесь сразу после команды смены скорости.
>
> Хорошо видно и само правило про «реальную, а не заданную» скорость: при переключении в режим HEAT байт уходит в `0x00` (вентилятор остановлен), хотя заданная скорость остаётся прежней. Горячей воды в теплообменнике нет, и фанкойл не даёт дуть холодным. При переключении в FAN вентилятор тут же возвращается к `0x06`.
...
> **Измерено на контроллере фанкойла AUX:** тут всегда `0x00`, в том числе при работающем вентиляторе на любой скорости. У фанкойла вентилятор трёхскоростной, ШИМ у него нет — и байт остаётся пустым.
...
> **Измерено на контроллере фанкойла AUX:** этому байту соответствует датчик, подписанный на разъёме платы как **ROOM**. Формула проверена в широком диапазоне, а не только около комнатной температуры: датчик опущен в очень холодную воду, байт 15 ушёл в `0x26` при `0x02` в байте 31, что даёт 6,2 °C. После извлечения из воды показания монотонно вернулись вверх через `0x27`, `0x28`, `0x29`, `0x2A`, `0x2B`, `0x2C`, `0x2D` — то есть 7,1 → 13,3 °C.
>
> Всё это время [**байт 17 (To)**](#packet_cmd_21_b17), которому соответствует датчик **PIPE**, оставался ровно на `0x39` и на охлаждение ROOM не реагировал вовсе. В обратном опыте (нагрев PIPE феном) точно так же не менялся байт 15. Датчики независимы, и соответствие байтов датчикам однозначное.
...
> **Измерено на контроллере фанкойла AUX:** здесь это наблюдение не подтверждается. В выключенном состоянии **байты 16 и 18 остаются нулевыми**, тогда как байты 15 и 17 содержат осмысленные температуры двух разных датчиков (`0x38` и `0x39`). У фанкойла термодатчика всего два — ROOM и PIPE, оба заведены на отдельный разъём, — и лишним байтам просто нечего показывать.
...
> **Измерено на контроллере фанкойла AUX:** здесь этот байт соответствует датчику, подписанному на разъёме платы как **PIPE** (датчик температуры теплообменника). Формула Brokly `T - 0x20` подтверждается.
>
> Проверка прямая: датчик PIPE нагрет строительным феном. На нагреве байт 17 шёл вверх (`0x45` → `0x4B` → `0x52` → `0x56` → `0x57`), после прекращения нагрева двинулся обратно и за 2 минуты монотонно, без пропусков прошёл `0x51` → `0x44` — все 14 промежуточных значений подряд, ровно как остывает нагретый феном датчик.
>
> Всё это время байт 15 (Tint) и байт 31 (Tid), которым соответствует датчик **ROOM**, держались в пределах 24,7..25,0 °C и на нагрев PIPE не реагировали вовсе.
>
> Калибровка по формуле сходится: в покое здесь `0x39`, что даёт `0x39 - 0x20` = 25 °C при комнатной температуре 24,8 °C по датчику ROOM. Пик нагрева `0x57` даёт 55 °C, что для строительного фена нормально.
>
> Отсюда же понятно, почему на сплитах формулу вывести не удавалось: байты внешнего блока (20, 22, 24) у фанкойла нулевые, а этот датчик стоит на теплообменнике внутреннего блока и от наружного воздуха не зависит.
>
> **Разрешение — 1 °C.** В отличие от температуры воздуха, у которой целая часть лежит в байте 15, а десятые — в отдельном [байте 31](#packet_cmd_21_b31), у этого датчика дробной ячейки нет: значение занимает один байт целиком. Видно и по данным — на остывании байт прошёл `0x51` → `0x44` через все 14 целых значений подряд, ни разу не показав ничего промежуточного. Байты 16, 18, 19 и 21, которые могли бы хранить дробную часть, на этом контроллере нулевые во всех пакетах.
...
> **Измерено на контроллере фанкойла AUX:** тут `0x00` во всех пакетах, при любом режиме и любой скорости вентилятора.
...
- контроллер фанкойла `AUX` - всегда `0x00` (0000 0000)
...
| значение | ошибка на дисплее | обстоятельства |
| -------- | ----------------- | -------------- |
| `0x00` | нет | нормальная работа |
| `0x07` | `E5` | внутренний блок сплита запущен без наружного |
| `0x0E` | `E4` | контроллер фанкойла с подключенными датчиками, но без двигателя вентилятора |

> **Измерено на контроллере фанкойла AUX:** плата на столе, питание 12 В, датчики ROOM и PIPE подключены и показывают корректную температуру, двигатель вентилятора не подключен. На дисплее `E4` («PG motor feedback is abnormal»), в байте 29 стабильно `0x0E` во всех пакетах.
>
> Отсюда следует, что **байт не является битовой маской неисправностей**: неисправность здесь ровно одна (нет обратной связи двигателя), а `0x0E` — это `0b0000 1110`, три установленных бита. Похоже на порядковый номер кода из внутреннего перечисления.
...
> **Измерено на контроллере фанкойла AUX:** дробная часть действительно хранится как обычное десятичное число. При медленном прогреве датчика ROOM здесь наблюдались значения от `0x01` до `0x09` включительно, и переход через десятые происходит с переносом в целую часть: `0x2B`+`0x09` (11,9 °C) сменяется на `0x2C`+`0x04` (12,4 °C). Значений выше `0x09` не встречалось.
...
##### Байт 32
Иногда встречается байт 32. Есть ощущение, что он появился в протоколе на постоянной основе в 2024-2025 годах. Всегда равен `0x00`. Возможно, появился только для упрощения расчета CRC.
...
| 21 | [PWR_LIM](#packet_cmd_11_b21)     | `0x00` |
...
> **Измерено на контроллере фанкойла AUX:** качания влево-вправо у него нет, и байт не принимает запись. В команде биты LR сбрасывались в `000`, но в ответном статусе байт неизменно возвращался как `0xE0`. То есть контроллер удерживает своё значение — так же, как он затирает неиспользуемые байты тела.
...
> **ВАЖНО:** Хотя в 5 бит вполне влезает значение 24, но для установки 24 часов нужно обнулить минуты таймера в [байте 14](#packet_cmd_11_b14) и часы здесь!
...
> **Измерено на контроллере фанкойла AUX:** комбинация с AUTO проверена — **AUTO с TURBO не совмещается**. При нажатии STRONG на ИК-пульте, когда скорость стояла в AUTO, контроллер одной посылкой выставил и бит TB, и заданную скорость HIGH: [**байт 13**](#packet_cmd_11_b13) сменился с `0xA0` на `0x20` одновременно с появлением `0x40` в байте 14.
>
> Обратный переход тоже наблюдался: снятие бита TB пришло вместе со сменой заданной скорости на LOW (байт 13 стал `0x60`). А вот переключение скорости между AUTO и HIGH бит TB не сбрасывает — он держался при обоих значениях байта 13. Это расходится с примечанием выше о том, что изменение скорости должно автоматически отключать TURBO.
...
> **Измерено на контроллере фанкойла AUX:** функции **MUTE здесь нет вовсе**. Бит MT не удерживается ни в одном режиме: контроллер команду подтверждает (и даже пищит, как на любую принятую команду), но в ответном статусе байт 14 возвращается нулевым.
>
> А вот **TURBO работает**, и на нём хорошо видно правило «реальная, а не заданная скорость». После установки бита TB байт 14 возвращается как `0x40`, заданная скорость в [**байте 13**](#packet_cmd_11_b13) остаётся прежней (MEDIUM), а [**байт 13 (FSPD)**](#packet_cmd_21_b13) пакета CMD = `0x21` уходит с `0x04` на `0x06` — вентилятор физически раскручивается до максимума. Снятие бита возвращает `0x04`.
...
| # бита | 7  | 6  | 5  | 4  | 3  | 2  | 1  | 0  |
| ------ | -- | -- | -- | -- | -- | -- | -- | -- |
| имя    | MD3| MD2| MD1| X  | iFL| SLP| FH | X  |
...
**Бит FH** указывает, в каких единицах должна отображаться температура на дисплее кондиционера. Если FH=1, то на дисплее температура будет отобрадаться в градусах Фаренгейта. Если FH=0, то будут использоваться градусы Цельсия.
...
> **Измерено на контроллере фанкойла AUX:** бит **SLP работает**. Байт 15 переходит с `0x20` на `0x24`, в статусе внешнего блока [**байт 11**](#packet_cmd_21_b11) синхронно меняется с `0x25` на `0x27` (бит SLP), а реальная скорость вентилятора падает на ступень — [**байт 13 (FSPD)**](#packet_cmd_21_b13) с `0x04` на `0x02`. Дисплей при включении функции гаснет целиком, вплоть до индикатора работы, и загорается обратно при снятии бита.
>
> Про режим **AUTO**: подтверждается, что отдельного бита у него нет — байт 15 становится `0x00`, а в статусе внешнего блока байт 11 равен `0x05`. Отдельного значка AUTO на дисплее нет, а сам контроллер сразу выбрал обогрев (в комнате было 19,7 °C при цели 24 °C) — слышно было, как переключились реле.
...
> **Измерено на контроллере фанкойла AUX:** подтверждается, что через UART бит не выставляется, а с ИК-пульта — выставляется. На пульте этой системы функции соответствует кнопка `FEELING`, и каждое её нажатие **переключает** бит 3 байта 15 в противоположное состояние.
>
> Заодно выяснилось, что этот же бит переключает и кнопка `HEALTH`. То есть наблюдение из [описания бита **HL2**](#packet_cmd_11_b18) о том, что HEALTH влияет на режим iFeel, подтверждается — но на этой модели влияние на бит iFL оказалось **единственным** следствием нажатия HEALTH.
>
> Полезно помнить, что пульт присылает не нажатую кнопку, а всё своё состояние целиком: вместе с изменением бита iFL в той же посылке приезжали и заданная скорость вентилятора, и целевая температура. Поэтому судить о назначении кнопки нужно по байту, который меняется **только** при её нажатии.
...
> **Измерено на контроллере фанкойла AUX:** байт контроллером не используется. В команде TYPE = `0x06` CMD = `0x01` сюда записывалось `0xFF`, в ответном статусе вернулся `0x00`. То есть контроллер не просто игнорирует значение, а обнуляет его. Точно так же ведут себя [**байт 17**](#packet_cmd_11_b17), [**байт 19**](#packet_cmd_11_b19) и [**байт 22**](#packet_cmd_11_b22).
>
> Там же проверены и все неиспользуемые биты тела: бит 6 [**байта 12**](#packet_cmd_11_b12), бит 5 [**байта 14**](#packet_cmd_11_b14), биты 4 и 0 [**байта 15**](#packet_cmd_11_b15), биты 4 и 3 [**байта 18**](#packet_cmd_11_b18), все шесть свободных битов [**байта 20**](#packet_cmd_11_b20). Каждый из них контроллер сбрасывает в `0`. Единственное исключение — бит 7 байта 18, см. его описание.
...
| # бита | 7  | 6  | 5  | 4  | 3  | 2  | 1  | 0      |
| ------ | -- | -- | -- | -- | -- | -- | -- | ------ |
| имя    | X  | TMR| POW| X  | X  | iCL| HL2| HL1-SFT|

Если **байт 18 (EN)** равен `0x00`, то сплит выключен (POWER OFF).

> **Измерено на контроллере фанкойла AUX:** **бит 7 здесь не пустой.** Это единственный из всех неиспользуемых битов тела, который контроллер принимает и хранит: отправили `0x80` — вернул `0x80`.
>
> Причём хранит он его **только при сброшенном бите POW**. Если отправить `0xA0` (POW вместе с битом 7), контроллер возвращает `0x20`: бит 7 сброшен, осталось только питание. То есть бит 7 подчиняется той же логике, что и **iCL**, который тоже задаётся при выключенном сплите.
>
> **Что делает бит — установлено наблюдением за дисплеем:** при `EN` = `0x80` на дисплее загорается **индикатор таймера, и только он**. Сброс байта в `0x00` его гасит, повторная установка `0x80` — зажигает снова. В байтах статуса внешнего блока при этом ничего не меняется (`MODE` остаётся `0x24`, `FSPD` — `0x00`), то есть на работу узлов бит не влияет.
>
> Стоит иметь в виду, что в таблице выше признаком таймера назван **бит 6 (TMR)**, а индикатор зажигает бит 7. Бит 6 отдельно на этом контроллере не проверялся, так что нельзя сказать, ошибка ли это в нумерации или у фанкойла своё назначение бита.
>
> Заодно выяснилось, что **функции iCLEAN здесь нет**: `EN` = `0x04` возвращается как `0x00`, а `0x84` — как `0x80`, то есть бит iCL срезается, а бит 7 сохраняется. Не удерживается и бит **HL2** (HEALTH) — контроллер команду подтверждает, но бит сбрасывает.
>
> Отсутствие подтверждено и со стороны ИК-пульта. На пульте этой системы есть кнопки `CLEAN` и `FUNGUSPROOF`, но на их нажатие контроллер **не отзывается даже звуком**. А писк он выдаёт на любую принятую команду, в том числе на команды по UART — то есть кнопка не распознана вовсе, а не принята и проигнорирована. Пульт универсальный, и часть кнопок на нём к этой модели просто не относится.
>
> Полезный побочный вывод: **писк контроллера — надёжный признак принятой команды**, и по нему видно разницу между «команда не дошла» и «команда принята, но ничего не делает».

**Бит TMR**. Это бит активации функции таймера.  
Установка `1` в этом бите активирует таймер. При этом задержка таймера устанавливается в [байте 13](#packet_cmd_11_b13) и [байте 14](#packet_cmd_11_b14) этого же пакета.
...
> **Измерено на контроллере фанкойла AUX:** сообщение Brokly подтверждается, причём условий два. Бит TMR удерживается, только если **питание включено** (бит POW) **и задержка непустая**. Попытка поднять один бит TMR при нулевых байтах 13 и 14 приводит к тому, что контроллер возвращает байт 18 нулевым — из-за этого бит легко принять за неиспользуемый. С командой `power=on` и задержкой 2:30 всё встаёт на место: байт 13 = `0x22`, байт 14 = `0x1E`, байт 18 = `0x60`.
>
> **Как выключить таймер.** Снять один бит TMR недостаточно: задержка остаётся в байтах 13 и 14, а индикатор таймера на дисплее продолжает гореть, потому что его зажигает [бит 7](#packet_cmd_11_b18) — он при этом остаётся установленным. Чтобы таймер погас полностью, нужно обнулить задержку в байтах 13 и 14 и снять оба бита, 6 и 7.
>
> **Предположение о двух таймерах.** Бит TMR хранится только при включенном питании, а бит 7 — только при выключенном, и оба зажигают один и тот же индикатор. Похоже, что бит 6 отвечает за таймер выключения, а бит 7 — за таймер включения, о возможности которого и говорится выше. Подтвердить не удалось: на стенде без нагрузки самопроизвольное включение по истечении задержки не наблюдалось.
...
Независисмо от наличия или отсутствия ионизатора в сплитке, после установки в `1` бита **HL2** переданной командой, сплит самостоятельно поднимает в `1` бит **HL1-SFT** в пакетах TYPE = `0x07` CMD = `0x11`. Далее пока функция работает, оба бита **HL1-SFT** и **HL2** остаются равными `1`, до выключенния функции.

Выключение функции происходит сбросом в `0` бита **HL2**. Бит **HL1-SFT** в данной команде также не трогаем. Сплит рулит им самостоятельно.
...
> **Измерено на контроллере фанкойла AUX:** **бит HL2 здесь не задействован вовсе.** Он не поднимается ни по UART (контроллер команду подтверждает, но бит сбрасывает), ни с ИК-пульта.
>
> Причём кнопка `HEALTH` на пульте нажатие отрабатывает: на собственном экране пульта функция зажигается, а контроллер в ответ переключает только [бит **iFL** байта 15](#packet_cmd_11_b15). Бит HL2 и бит HL1-SFT остаются нулевыми.
>
> Похоже, ионизатора в этой модели нет, а пульт универсальный и показывает функции, которых у конкретного блока может не быть. Косвенно это подтверждают кнопки `CLEAN` и `FUNGUSPROOF` того же пульта: на них контроллер не отзывается даже звуком.

**Бит HL1-SFT** - Ранее было не понятно назначение этого бита. Единственное, что выяснили, что он как-то связан с ионизатором и сплит рулит им самостоятельно: появление в этом бите `1` связано с включением ионизатора (функция `HEALTH`).
В 2025 году появились кондиционеры с функцией "мягкого" потока воздуха: кнопка `SOFT`, кондиционер полностью закрывает жалюзи, включает вентилятор на минимум и рассеивает холодный воздух через большое количество отверсий в жалюзи. Для этих кондиционеров этот бит означает включение этой функции.
Скорее всего, этот бит отвечает за закрытие шторки при работающем вентиляторе. Для функции `HEALTH` поведение шторки аналогичное.
...
> **Измерено на контроллере фанкойла AUX:** логика здесь **инвертирована**, как у Rovex. Проверено напрямую: при питании, включенном битом POW байта 18, состояние бита DS удерживалось по 12 секунд в каждом положении и дисплей наблюдался глазами.
>
> - **DS = 1** (байт 20 = `0x10`) — дисплей полностью гаснет;
> - **DS = 0** (байт 20 = `0x00`) — дисплей светится.
>
> То есть для этого контроллера в компоненте `aux_ac` нужен `display_inverted`. Инвертированное управление дисплеем встречается не только у Rovex. 
...
> **Измерено на контроллере фанкойла AUX:** функции «антиплесень» здесь нет — бит MD не удерживается, байт 20 возвращается нулевым. Бит **DS при этом рабочий**, но с инвертированной логикой (см. выше). То есть из всего байта 20 у этого контроллера задействован только бит дисплея.
...
> **Измерено на контроллере фанкойла AUX:** механизм подтверждён целиком. Команда на 23,5 °C ушла с байтом 10 = `0x7F`, установленным битом TD байта 12 (`0x8A`) и **байтом 22 = `0x00`**. В ответном статусе байт 22 стал **`0x05`**, а байты 10 и 12 вернулись без изменений.
>
> При этом на дисплее контроллера отобразилось `23` — половину градуса он не показывает, хотя по протоколу принимает и хранит.
